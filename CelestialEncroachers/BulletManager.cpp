...
bool BulletManager::CheckCollision(const sf::FloatRect& aCollisionBox)
{

	for (int i = 0; i < myBullets.Count(); i++)
	{
		if (myBullets[i].CheckOwner() == true)
		{
			if (myBullets[i].GetCollisionBox().intersects(aCollisionBox))
			{
				myBullets.RemoveAt(i);
				return true;
			}
